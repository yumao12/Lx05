...
namespace DAL
{
    public class Activity
    {
        ...
        public IEnumerable<Model.Activity> GetVerifyPage(Model.Page page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over(order by endTime desc) as num, activityId,activityName,endTime,activityPicture,summary,activityVerify,userName,recommend,recommendtime,endTime>=now() as activityStatus from activity " +
        ...
        public IEnumerable<Model.ActivityName> GetActivityNames()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select actinityId,activityName from activity";
        ...
        public int Delete(int id)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "delete from activiy where activityid=@id";
        ...
        public int UpdateRecommend(Model.Activity activity)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update activity set  recommend=@recommend,recimmendTime=@recommendTime where activityid=@activityId";
